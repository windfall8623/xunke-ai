...
7. 网站课程接口处理纲要、按需课时、阅读记录与客观练习。技能中的聊天式补讲、单题反馈和调整建议只在支持对话的调用环境中使用，不能假定网站已提供自由课程对话接口。
...
1. **结构化生成与存储**：`CourseDraft` / `LessonDraft` 校验教学草案；应用分配真实身份，将课程和课时分别存入 `learning_courses`、`learning_course_lessons`，再投影为课程 HTTP DTO。来源模式、资料指纹、内容版本、技能版本和哈希保存在服务端。
