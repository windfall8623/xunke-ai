<div align="center">
  ...
  <p>围绕知识库问答与智能练习，学习 Agent 编排、RAG 检索和 AI 应用工程。</p>
</div>
...
你可以上传自己的资料，围绕内容进行多轮问答，用学习空间串联目标、练习、错题与复习；也可以阅读和修改代码，学习一个 Agent 应用如何组织检索、生成、校验、异步任务与费用管理。
...
| 学习应用设计 | 问答转练习、学习事件、规则判分、评分复核与复习安排 |
...
脚本会创建 `deploy/.env` 并生成数据库密码与应用密钥。要使用真实问答、出题和资料检索，按下方说明填写 LLM 与 Embedding 配置；暂不配置模型时，也可以启动网页体验注册、登录和预置题目。
...
LLM 负责问答、出题和报告；Embedding 负责资料向量化。使用知识库功能时，需要分别配置这两类模型。
