...
export async function installTestApi(
  page: Page,
  options: { guest?: boolean; evaluator?: boolean; admin?: boolean; longName?: boolean; courses?: boolean } = {},
) {
  const baseDataset = {
    dataset_id: 'data-1',
    version: 1,
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.startsWith('/eval/') && !['admin', 'evaluator'].includes(state.user.role))
      return fail(403, '需要评测权限')
    if (state.user.role !== 'admin' && (
      (path === '/eval/runs' && method === 'POST') ||
      /^\/eval\/runs\/[^/]+\/resume$/.test(path)
    )) return fail(403, '只有管理员可执行系统模型评测')
