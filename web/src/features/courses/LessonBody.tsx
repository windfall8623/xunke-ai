...
export function LessonBody({ blocks, helpDisabled, onEvidence, onExplain }: LessonBodyProps) {
  return (
    <section className="course-lesson-body" aria-label="本课正文">
      {blocks.map((block, index) => (
        <section className={`course-block course-block-${block.type}`}
          aria-label={`${blockLabels[block.type]} ${index + 1}`} key={index}>
