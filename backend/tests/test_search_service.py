...
def mock_settings_enabled():
    ...
    settings.provider_timeout_seconds = 45
    return settings
...
async def test_fetch_returns_empty_when_disabled(mock_settings_disabled):
    """配置关闭时应返回空字符串"""
    with patch(
        "app.services.search_service.get_settings", return_value=mock_settings_disabled
    ):
...
async def test_fetch_returns_empty_when_no_api_key(mock_settings_no_key):
    ...
    with patch(
        "app.services.search_service.get_settings", return_value=mock_settings_no_key
    ):
        from app.services.search_service import fetch_knowledge_context
...
async def test_fetch_returns_context_on_success(mock_settings_enabled):
    ...
    with (
        patch(
            "app.services.search_service.get_settings",
            return_value=mock_settings_enabled,
        ),
        patch(
            "app.services.search_service._build_agent",
            return_value=(mock_agent, MagicMock()),
        ),
    ):
        from app.services.search_service import fetch_knowledge_context
