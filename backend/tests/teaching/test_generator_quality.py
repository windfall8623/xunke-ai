...
@pytest.mark.asyncio
async def test_personal_generation_and_schema_repair_preserve_meter_provenance(
    monkeypatch, valid_v2_course, topic_spec, teaching_budget,
):
    from app.services import provider_meter

    chat = ScriptedChat(reply("invalid-json"), reply(valid_v2_course))
    calls = []

    async def metered(stage, call, **kwargs):
        calls.append((stage, kwargs))
        return await call()

    monkeypatch.setattr(provider_meter, "call_external", metered)
    llm = provider_meter.MeteredChat(chat, purpose="course_outline", output_upper=3000,
                                     config_source="user", api_key="synthetic-private-key")
    generator = CourseGenerator(llm, llm, model_configuration={"config_source": "user"})
    result = await generator.outline(topic_spec, TeachingMaterial(), budget=teaching_budget)
    assert [kwargs["purpose"] for _, kwargs in calls] == ["course_outline", "course_teaching_repair"]
    assert all(stage == "llm" and kwargs["config_source"] == "user" for stage, kwargs in calls)
    assert all(kwargs["api_key"] == "synthetic-private-key" for _, kwargs in calls)
    assert all(kwargs["output_upper"] == 3000 for _, kwargs in calls)
    assert llm.purpose == "course_outline"  # a repair must not mutate the shared wrapper
    assert result["effective_config"]["config_source"] == "user"
    assert result["effective_config"]["generation_attempts"] == 2


@pytest.mark.asyncio
async def test_personal_provider_failure_does_not_use_generation_repair_or_fallback(
    monkeypatch, topic_spec, teaching_budget,
):
    from app.services import provider_meter

    failure = AppError(422, "user_llm_failed", "Synthetic personal model failure")
    chat = ScriptedChat(failure, reply("must-not-run"))

    async def metered(stage, call, **kwargs):
        assert kwargs["config_source"] == "user"
        return await call()

    monkeypatch.setattr(provider_meter, "call_external", metered)
    llm = provider_meter.MeteredChat(chat, config_source="user", api_key="synthetic-private-key")
    with pytest.raises(AppError) as error:
        await CourseGenerator(llm, llm).outline(topic_spec, TeachingMaterial(), budget=teaching_budget)
    assert error.value is failure
    assert len(chat.calls) == teaching_budget.snapshot().llm_calls == 1
