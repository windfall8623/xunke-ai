...
async def create_course(actor, body, key):
    ...
    frozen = await course_teaching.frozen_request_fields("outline", body, owner_id=owner)
    ...
    await course_teaching.require_teaching_available(
        body.teaching_mode, body.request_quality_review, owner_id=owner,
    )
...
async def retry_outline(actor, course_id, key):
    ...
    frozen = await course_teaching.frozen_request_fields("outline", spec, owner_id=owner)
    ...
    if old:
        return await course_read.get_task(owner, old["task_id"])
    await course_teaching.require_teaching_available(
        spec.teaching_mode, spec.request_quality_review, owner_id=owner,
    )
...
async def generate_lesson(actor, course_id, lesson_id, body, key):
    ...
    frozen = await course_teaching.frozen_request_fields(
        "lesson", spec, owner_id=owner, request_quality_review=body.request_quality_review,
    )
    ...
    if old:
        return await course_read.get_task(owner, old["task_id"])
    await course_teaching.require_teaching_available(
        spec.teaching_mode, body.request_quality_review, owner_id=owner,
