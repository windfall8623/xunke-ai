...
def _build_agent():
    ...
    llm = create_chat_model(settings, temperature=0.1, callbacks=[tool_logger])
...
async def fetch_knowledge_context(user_input: str) -> str:
    ...
    try:
        ...
        logger.debug(
            "search_agent_built", elapsed_ms=round((time.monotonic() - t0) * 1000)
        )
        ...
        # 提取 Agent 最终回复
        content = response_text(result["messages"][-1])
        if not content:
        ...
        if len(content) > MAX_CONTEXT_LENGTH:
            ...
            logger.info(
                "search_context_truncated",
                original_length=len(result["messages"][-1].content),
            )
