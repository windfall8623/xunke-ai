...
def _call_image_model_sync(prompt: str) -> str:
    ...
    dashscope.base_http_api_url = image.base_url
...
async def generate_images_for_quiz(
    ...
    if remaining <= 0:
        return (
            {},
            f"今日生成图片次数已用完（{settings.image_gen_daily_limit} 张/天），本次出题未生成配图",
        )
    ...
    for question_id, url in results:
        ...
        try:
            await image_repository.log_image_generation(
                user_id, quiz_id, question_id, url
            )
        except Exception as e:
