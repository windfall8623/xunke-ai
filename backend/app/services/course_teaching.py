...
from app.services.user_llm_config_service import configuration_required, resolve_actor_llm_config
...
async def require_model(owner_id, *, settings=None, conn=None):
...
async def capabilities(owner_id):
    ...
    if not settings.course_enabled:
        ...
        try:
            config = await resolve_actor_llm_config(owner_id, settings=settings)
        except AppError as exc:
            if exc.code not in {"llm_configuration_required", "user_llm_secret_missing", "user_llm_key_unreadable"}:
                raise
            reason = exc.code
        else:
            if not config.configured:
...
async def require_teaching_available(mode, request_quality_review, *, owner_id, schema_version=V2):
    if mode != "guided" and not request_quality_review:
        return
    if schema_version != V2:
        raise AppError(409, "course_quality_unavailable", "旧版课程保留原有生成方式，暂不支持教学核对")
    if "guided" not in (await capabilities(owner_id))["teaching_modes"]:
        raise AppError(409, "teaching_agents_unavailable", "协作教学暂未就绪，请选择快速生成或稍后重试")


async def frozen_request_fields(kind, spec, *, owner_id, request_quality_review=None, inherited=None, conn=None):
    ...
    if inherited and inherited.get("teaching_model") is not None:
        # Automatic preloading keeps the accepted outline's model identity. The
        # new task still resolves current credentials/role on claim and fails
        # closed if that identity is no longer available.
        teaching_model = inherited["teaching_model"]
    else:
        config = await require_model(owner_id, settings=settings, conn=conn)
        teaching_model = {
...
def require_frozen_runtime(job, generator, policy):
    ...
    # Older main tasks have no provenance field. Do not rewrite their identity;
    # new tasks pin it explicitly so role changes cannot swap user/system models.
    identity_fields = IDENTITY_FIELDS + (("config_source",) if isinstance(frozen, dict) and "config_source" in frozen else ())
