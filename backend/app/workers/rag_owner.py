...
class OwnerWorker:
    ...
    async def _worker_heartbeat(self):
        # Real workers create owner-specific teaching adapters at dispatch, even
        # when no shared system chat model is configured. This heartbeat reports
        # adapter support; the API checks the requesting owner's configuration.
        teaching_ready = get_settings().course_enabled and (
            self.runtime_resolver is not None
            or self.course_generator is not None
            and getattr(self.course_generator, "reviewer", None) is not None
        )
        if self.role != "owner":
